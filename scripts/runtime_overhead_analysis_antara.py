#!/usr/bin/env python3
"""
Runtime overhead analysis (NPU/Antara): produces runtime_overhead_antara.pdf.

Same methodology as runtime_overhead_analysis.py but for the NPU environment.
...
Usage (inside Docker):
    python3 scripts/runtime_overhead_analysis_antara.py
    python3 scripts/runtime_overhead_analysis_antara.py --duration 30 --reps 2
    python3 scripts/runtime_overhead_analysis_antara.py --replot
"""
import argparse
import csv
import datetime
...
OVERHEAD_DIR = os.path.join(RESULTS_DIR, "runtime_overhead_antara")
SCHEDULE = os.path.join(PROJECT_DIR, "tests", "steady_state_views_schedule_npu.yaml")
EXECUTOR = os.path.join(PROJECT_DIR, "schedule_executor_main.py")
PYTHON = sys.executable

OUT_PDF = os.path.join(RESULTS_DIR, "runtime_overhead_antara.pdf")
...
def reap_lingering_executors(cooldown_sec: float = 5.0):
        ...
        while proc.poll() is None and time.time() < deadline:
            ...
            try:
                ...
                if time.time() >= warmup_until:
                    ...
                    rss_samples.append(rss / (1024.0 ** 2))
...
def _run_sweep(args):
    ...
    for mi, m in enumerate(MODES):
        ...
        for rep in range(args.reps):
            ...
            if metrics is None:
                print(f"     [warn] no metrics from {csv_path}")
                continue
            ...
            reap_lingering_executors(args.cooldown_sec)
...
def main():
    ...
    if args.replot:
        ...
        started = _now_iso()
        print(f"[Overhead] {started} starting steady-state overhead sweep (Antara/NPU)")
