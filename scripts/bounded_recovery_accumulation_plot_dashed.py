#!/usr/bin/env python3
"""
Generate bounded_recovery_analysis_accumulation_dashed.pdf — stacked bar chart
showing T_detect (orange) + T_post (dark blue) per workload scenario, with
a horizontal dashed line indicating the theoretical upper bound
T_recovery <= T + N_cand * (T_v + delta) + T_stable.

Usage:
    python scripts/bounded_recovery_accumulation_plot_dashed.py
    python scripts/bounded_recovery_accumulation_plot_dashed.py --bound 30
    python scripts/bounded_recovery_accumulation_plot_dashed.py \
        --T 3 --Tv 3 --N-cand 2 --delta 1 --T-stable 18
"""
...
OUT_PDF = os.path.join(RESULTS_DIR, "bounded_recovery_analysis_accumulation_dashed.pdf")
...
def main() -> None:
    ...
    parser.add_argument("--bound", type=float, default=None,
                        help="Override: theoretical upper bound (seconds). "
                             "If not set, computed from --T, --Tv, --N-cand, "
                             "--delta, --T-stable.")
    ...
    parser.add_argument("--N-cand", type=int, default=2,
                        help="Number of candidate placements evaluated. Default: 2.")
    parser.add_argument("--delta", type=float, default=1.0,
                        help="Switching + warm-up overhead delta (s). Default: 1.")
    parser.add_argument("--T-stable", type=float, default=18.0,
                        help="Stabilization time after applying a valid "
                             "placement (s). Default: 18.")
    ...
    # Theoretical upper bound (horizontal dashed line)
    ax.axhline(
        y=bound,
        color="#c0392b",
        linestyle="--",
    ...
    ax.set_ylim(0, max(total.max(), bound) * 1.20)
