        ...
        for hid in list(getattr(self, 'headless_ids', []) or []):
            ...
            if "yolov4" in model:
                ...
                if execution == "gpu":
                    ...
                    # NPU workers are real subprocesses, so they need
                    # multiprocessing-flavoured queues / event.
    ...
    def start_view_process(self, view_name):
        ...
        if "yolov4" in model:
            ...
            if execution == "gpu":
                ...
                # NPU workers run in their own subprocess, so frame/result queues
                # and the shutdown event must be the multiprocessing flavour.
                # NOTE: this re-creates the queues per phase, which means NPU
                # workers do NOT inherit the persistent input backlog the GPU
                # branch designed for cold-start V(t) measurement during
                # mode 0 phase transitions. This is a known limitation of the
                # current NPU port; CPU/GPU workers still get the persistent
                # queues created in initialize_state_variables.
                frame_queue = mp.Queue(maxsize=2)
                output_queue = mp.Queue(maxsize=1)
                shutdown_event = mp.Event()
