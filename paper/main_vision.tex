...
Modern edge platforms combine heterogeneous accelerators and co-resident workloads to balance throughput, latency, and power consumption~\cite{mobilint-aries}. Prior work selects device placements and schedules from several signals: isolated accuracy and latency profiles~\cite{tod,partitiontuner,ngs,rl-scheduler}, and measured or predicted GPU utilization, which is a common scheduling and placement signal in GPU cluster management~\cite{gandiva,horus}. Interference-aware systems such as iGniter~\cite{igniter} go further by predicting contention-induced latency, but they target latency SLOs rather than delivery-time streaming accuracy, and do not consider heterogeneous GPU--NPU placement. These studies primarily optimize throughput, latency, utilization, or static accuracy. In contrast, we study deployment-time streaming accuracy, show that accelerator sharing can reverse placement decisions, and quantitatively evaluate the placements implied by these selection criteria under contention.
...
We distinguish foreground streams from a GPU-pinned co-tenant workload. We evaluate three co-tenant levels, summarized in Table~\ref{tab:cotenants}. The levels span light CNN inference (\Lcnn{}), mixed CNN+LLM inference (\Llm{}), and a GPU-intensive vision-language workload (\Lvlm{}). All co-tenants execute on CUDA. For contention sweeps, we additionally vary the number $k$ of concurrent GPU-pinned ResNet50 co-tenant instances (the \emph{$k$-sweep}); increasing $k$ raises the measured foreground GPU deadline-miss rate from about $24\%$ to $67\%$ at $N{=}4$.
...
Each measurement uses a $14$~s evaluation window following a $30$-frame warm-up period. Results are averaged over at least three independent repetitions, with additional repetitions at the near-deadline boundary operating point. Statistical significance is reported only for single-camera comparisons, where the $24$ Argoverse-HD logs provide paired samples for a Wilcoxon signed-rank test. Multi-stream contention results are instead summarized as means over repeated runs, with run-to-run variability reported in Sec.~\ref{sec:discussion:limitations}, because three to ten repetitions do not support a meaningful hypothesis test.
...
Beyond the placement configurations above, we evaluate the placement that common selection criteria would choose at each contention level. \emph{Isolated-accuracy selection} (P1) places all streams on the device with the higher isolated single-stream sAP, reflecting vendor-style FP32 preference. \emph{Latency-profiling selection} (P2) places all streams on the lowest-latency device that meets the frame budget, following transprecision-style profiling~\cite{tod}. \emph{Utilization-threshold selection} (P3) migrates streams to the NPU, large-object-first in the same order as the Oracle, once measured mean GPU utilization exceeds a threshold $\theta\in\{70,80,90\}\%$, reflecting the common use of GPU utilization as a scheduling and placement signal~\cite{gandiva,horus}. Finally, \emph{deadline-miss-aware selection} (P4) moves all streams to the NPU once the foreground GPU deadline-miss rate exceeds the measured homogeneous crossover; it is not a competing prior-work policy but a policized form of our own reporting recommendation, included as a diagnostic.
...
\caption{Per-size sAP loss versus the measured GPU deadline-miss rate (All-GPU, $N{=}4$, ResNet50 $k$-sweep, where $k$ is the number of GPU-pinned ResNet50 co-tenants; mean over repeated runs). Large-object loss rises steeply while small-object loss stays nearly flat. The shaded band marks the crossover region near $48\%$ deadline-miss rate (Fig.~\ref{fig:sweeps}a). All-NPU per-size loss is approximately zero across the sweep and is omitted.}
...
The reversal is not caused by contention alone. Table~\ref{tab:main} shows that the outcome depends on which path is bottlenecked. Under the light CNN co-tenant, GPU placements remain competitive and no clear reversal is observed. Under the \Llm{} co-tenant, both paths are stressed: GPU deadline misses reach $83\%$ and NPU deadline misses reach $76\%$, so All-GPU still outperforms All-NPU in worst-stream sAP despite substantial GPU contention. The Oracle ($0.063$) is essentially tied with All-GPU ($0.062$) in this setting, indicating that mixed placements offer no benefit when both execution paths are bottlenecked. In contrast, under the \Lvlm{} co-tenant, GPU deadline misses reach $100\%$ while NPU deadline misses remain $1\%$, and the ranking reverses in favor of All-NPU. In our measurements, host CPU utilization rises from $17\%$ to $93\%$ under the \Llm{} co-tenant, stalling NPU host-side post-processing.
%
...
The same sweep also explains why we use deadline misses, rather than aggregate GPU utilization, as the contention coordinate. After a ResNet50 co-tenant is introduced, average GPU utilization remains nearly flat at about $68\%$, while the foreground GPU deadline-miss rate rises from $24.3\%$ to $67.3\%$. In aggregate, mean utilization never exceeds $70\%$ at any sweep point (peaking at $69.7\%$, with p95 at most $86\%$), so common utilization thresholds of $80$--$90\%$ would never fire in this regime (Table~\ref{tab:policy-comparison}). The placement reversal therefore follows missed real-time deadlines rather than aggregate GPU utilization.
...
To rule out a strawman comparison, Table~\ref{tab:policy-comparison} reports the placement each selection criterion chooses at representative contention levels, together with its worst-stream regret against the post-hoc Oracle over the same restricted placement family. Isolated-accuracy (P1) and latency-profiling (P2) selection both pick All-GPU at every operating point, since the GPU wins isolated sAP ($0.196$ vs.\ $0.186$) and isolated latency ($8.3$ vs.\ $10.0$~ms, both within the $33.3$~ms budget); their regret grows from $+0.005$ at low contention to $+0.012$ at the heaviest ResNet50 point and reaches $+0.068$ under the \Lvlm{} co-tenant. Utilization-threshold selection (P3) fails differently. Mean GPU utilization peaks at $69.7\%$ at the lightest sweep point and stays near $68\%$ thereafter, while foreground deadline misses climb from $24\%$ to $67\%$, so thresholds of $80$--$90\%$ never fire on the sweep. A $70\%$ threshold is borderline only at the lightest point, where run-to-run variability ($69.7\pm2.9\%$, exceeding $70\%$ in $4$ of $10$ runs) flips its selection between All-GPU ($+0.005$ regret) and All-NPU ($+0.020$ regret). The threshold fires decisively only once the GPU is fully saturated by the \Lvlm{} co-tenant. A utilization threshold is therefore a lagging signal that reacts only after collapse, and an unstable one near its boundary. Deadline-miss-aware selection (P4) tracks the reversal on the sweep with near-zero regret past the crossover, but mis-selects All-NPU under the \Llm{} co-tenant ($+0.020$ regret), where host-CPU contention stalls the NPU path as well. Placement selection therefore requires deadline-miss visibility on \emph{both} execution paths, which isolated profiling and aggregate utilization do not provide.

\begin{table}
\centering
\caption{Worst-stream sAP and regret (in parentheses, vs.\ the post-hoc Oracle) of the placement chosen by each selection criterion at $N{=}4$. Isolated-accuracy (P1) and latency-profiling (P2) selection choose All-GPU everywhere. The utilization threshold (P3, $\theta{=}80\%$ shown) fires only at full GPU saturation; $\theta{=}90\%$ behaves identically, while $\theta{=}70\%$ is additionally unstable at the lightest point. Deadline-miss-aware selection (P4) tracks the reversal but errs at \Llm{}, where the NPU path is also stalled.}
...
\bottomrule
\multicolumn{5}{p{0.93\linewidth}}{\footnotesize $^\dagger$At \Llm{}, the utilization input is a co-tenant-only proxy ($49.5\%$), as All-GPU utilization under this co-tenant was not directly logged. Even if utilization exceeded the threshold, the migrated All-NPU placement would yield the same $+0.020$ regret as P4, so the conclusion is unchanged.} \\
\end{tabular}
\end{table}
...
We do not propose a general robustness framework. Instead, we distill our findings into three reporting recommendations for shared platforms. First, report sAP under co-tenancy localized to the shared accelerator, not only in isolation. Second, report worst-stream sAP alongside mean sAP. Third, report foreground deadline-miss rates on both the GPU and NPU paths, together with the contention sweep and crossover point; keying on GPU deadline misses alone mis-selects the placement under the \Llm{} co-tenant (Table~\ref{tab:policy-comparison}). These measurements expose the reversal and its severity. The standard isolated single-stream evaluation protocol hides both. Table~\ref{tab:reporting} summarizes the contrast.
