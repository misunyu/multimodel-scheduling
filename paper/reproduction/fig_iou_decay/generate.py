...
ROOT = Path(__file__).resolve().parent  # [reproduction] path adjusted
DECAY_CSV = ROOT / "data/b1_temporal_iou_decay.csv"  # [reproduction] path adjusted
PROXY_CSV = ROOT / "data/b1_staleness_sap_proxy.csv"  # [reproduction] path adjusted
OUT_PDF = ROOT / "b1_iou_decay_percent.pdf"  # [reproduction] path adjusted
# PAPER_PDF = ROOT / "paper/figures/b1_iou_decay_percent.pdf"  # [reproduction] disabled: no writes outside folder
...
def main():
    ...
    fig.savefig(OUT_PDF)
    plt.close(fig)
    # shutil.copyfile(OUT_PDF, PAPER_PDF)  # [reproduction] disabled: no writes outside folder
    print(f"wrote {OUT_PDF}")
    # print(f"copied -> {PAPER_PDF}")  # [reproduction] disabled: no writes outside folder
