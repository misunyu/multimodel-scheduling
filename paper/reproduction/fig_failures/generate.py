# =============================================================================
# REPRODUCTION HEADER  (added for the reproduction package — logic unchanged below)
# -----------------------------------------------------------------------------
# Figure: fig:failures  (qualitative INT8-NPU quantization failures)
#
# This file is a VERBATIM copy of the original generator:
#     paper/results_data/fig1_failures/generate_fig1.py
# Nothing in the logic below has been modified.
#
# ** HARDWARE-DEPENDENT — CANNOT BE RUN IN THIS REPRODUCTION ENVIRONMENT. **
# The generator performs LIVE inference on real devices:
#   * a CUDA GPU running YOLOv11s in FP32 (via `ultralytics` + `device="cuda"`), and
#   * a Mobilint NPU running the same model in INT8 (via the repo's NPU modules).
# It also depends on repo-internal modules that it imports through sys.path
# manipulation relative to its ORIGINAL location (it expects sibling directories
# such as `minimal_pipeline/_step_d_common.py`, `step0_compare_devices`, the pinned
# `results/rev7_pin.json` mxq path, and the local Argoverse-HD dataset).
#
# Therefore it MUST be run from its ORIGINAL location, on the original platform,
# with GPU + NPU present — it will NOT import or run from this reproduction folder,
# and it is NOT meant to be run here. See ./README.md for details.
#
# Original (non-runnable here) invocation, from the original directory:
#     cd paper/results_data/fig1_failures
#     python generate_fig1.py     # writes paper/figures/vis_failure_examples.pdf
#
# The exact rendered PDF (paper/figures/vis_failure_examples.pdf) is provided in
# this folder as a byte-for-byte copy: ./vis_failure_examples.pdf
# =============================================================================
