...
| ~~Q3 2-gen 회복(llama1b만; qwen2_vl CPU 금지) 6/32~~ | — | — | **정정됨 → gate A** |

> **정정 (2026-07-28, docs/gate_a_device_constraints.md)**: 위 2-gen 행은 **틀렸다.** `qwen2_vl` CPU
> 금지 제약은 **생성기 전용**이고 런타임·실험엔 없다. 제약을 빼면 both-CPU가 후보로 등장하며(논문의
> feasible 회복 배치, V=0), rank = **16/64**(§IV-A fg4) 또는 **12/32**(논문 fg3, "rank twelve" 재현).
> "llama1b만, rank 6"은 생성기 과제약 아티팩트였다.
